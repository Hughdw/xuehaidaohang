  ...
  var oButton = {
    ...
    bindEvent : function(linkBtn,collapse,type,seted,cancelFn) {
      ...
      jqCollapse.on('show.bs.collapse',function() {
        ...
        // 按钮的文字切换为 取消
        jqPrev.addClass('account-tit-active').find('.collapse-link').text(self._btnTxt[aBtnType][0]);
        // 控制 绑定手机/邮箱 的内容显示
        if (bSeted) {
          // 信息是已设置状态，可以进行修改。
      ...
      jqCollapse.on('hide.bs.collapse',function() {
        ...
        if (bSeted) {
          // 按钮的文字切换为 修改/更换绑定
          jqPrev.removeClass('account-tit-active').find('.collapse-link').text(self._btnTxt[aBtnType][2]);
        } else {
          // 按钮的文字切换为 设置
          jqPrev.removeClass('account-tit-active').find('.collapse-link').text(self._btnTxt[aBtnType][1]);
        }
        // 取消按钮触发的函数
